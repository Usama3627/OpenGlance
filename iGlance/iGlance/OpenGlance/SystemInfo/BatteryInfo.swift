...
class BatteryInfo {
    ...
    private var batteryConnectionOpen = false
    ...
    deinit {
        if batteryConnectionOpen && skBattery.close() != kIOReturnSuccess {
            DDLogError("Closing a connection to the battery failed")
        }
    }
    ...
    func getBatteryHealth() -> Float {
        if let capacities = readBatteryCapacities(), capacities.design > 0 {
            let health = Float(capacities.maximum) / Float(capacities.design)
            DDLogInfo("Read maximum capacity \(capacities.maximum) and design capacity \(capacities.design) from IORegistry")
        ...
        return min(max(Float(maxCap) / Float(designCap), 0), 1)
    }
    ...
    func getBatteryCycles() -> Int {
        if let cycleCount = readSmartBatteryProperties()?["CycleCount"] as? NSNumber {
            DDLogInfo("Read the cycle count of the battery from IORegistry: \(cycleCount.intValue)")
    ...
    /// Reads modern AppleSmartBattery properties. SystemKit's legacy SMC
    /// capacity calls can return zero on Apple Silicon.
    ...
    private func readBatteryCapacities() -> (maximum: Int, design: Int)? {
        guard let properties = readSmartBatteryProperties(),
              let design = properties["DesignCapacity"] as? NSNumber else {
            return nil
        }

        // NominalChargeCapacity is the calibrated full-charge capacity shown
        // by macOS. AppleRawMaxCapacity is retained as a firmware fallback.
        let maximum = (properties["NominalChargeCapacity"] as? NSNumber)
            ?? (properties["AppleRawMaxCapacity"] as? NSNumber)
        guard let maximum else { return nil }
        return (maximum.intValue, design.intValue)
    }
