...
class CpuViewController: MainViewViewController {
    ...
    private func setupProgrammaticLayout() {
        graphSelector.translatesAutoresizingMaskIntoConstraints = false
        graphSelector.widthAnchor.constraint(equalToConstant: 140).isActive = true
        
        usageColorWell.translatesAutoresizingMaskIntoConstraints = false
        usageColorWell.widthAnchor.constraint(equalToConstant: 44).isActive = true
        usageColorWell.heightAnchor.constraint(equalToConstant: 24).isActive = true
        
        secondaryColorWell.translatesAutoresizingMaskIntoConstraints = false
        secondaryColorWell.widthAnchor.constraint(equalToConstant: 44).isActive = true
        secondaryColorWell.heightAnchor.constraint(equalToConstant: 24).isActive = true
        
        graphWidthSlider.translatesAutoresizingMaskIntoConstraints = false
        graphWidthSlider.widthAnchor.constraint(equalToConstant: 150).isActive = true
        
        let widthStack = NSStackView(views: [graphWidthSlider, graphWidthLabel])
        widthStack.orientation = .horizontal
        widthStack.spacing = 8
        widthStack.translatesAutoresizingMaskIntoConstraints = false
        
        let sections = [
            SettingsFormSection(title: "System Monitor", rows: [
                SettingsFormRow(left: cpuTempCheckbox, right: nil),
                SettingsFormRow(left: cpuUsageCheckbox, right: nil)
            ]),
            SettingsFormSection(title: "Usage Graph Style", rows: [
                SettingsFormRow(label: "Graph Type", subtitle: "Select how CPU usage metrics are plotted", control: graphSelector),
                SettingsFormRow(left: usageGraphBorderCheckbox, right: nil),
                SettingsFormRow(label: "Primary Color", subtitle: "Main color used for plotting CPU usage", control: usageColorWell),
                SettingsFormRow(left: colorGradientCheckbox, right: nil),
                SettingsFormRow(label: "Secondary Color", subtitle: "Gradient secondary color (if enabled)", control: secondaryColorWell),
                SettingsFormRow(label: "Line Graph Width", subtitle: "Width in pixels of the usage line graph", control: widthStack)
            ])
        ]
        SettingsFormBuilder.build(in: self.view, sections: sections)
    }
    ...
    @IBOutlet private var cpuTempCheckbox: NSButton! {
        didSet {
            ...
            configureTemperatureAvailability()
        }
    }
    ...
    private func configureTemperatureAvailability() {
        guard AppDelegate.systemInfo.cpu.getCpuTemp() < 0 else { return }

        cpuTempCheckbox.state = .off
        cpuTempCheckbox.isEnabled = false
        cpuTempCheckbox.title = "CPU temperature — Unavailable on this Mac"
        cpuTempCheckbox.toolTip = "This Mac does not expose a supported CPU temperature sensor to OpenGlance."
        AppDelegate.userSettings.settings.cpu.showTemperature = false
    }
