...
class SettingsViewController: MainViewViewController {
    ...
    private func setupProgrammaticLayout() {
        ...
        logoStack.addArrangedSubview(logoImage)
        logoImage.translatesAutoresizingMaskIntoConstraints = false
        logoImage.widthAnchor.constraint(equalToConstant: 48).isActive = true
        ...
        let nameLabel = NSTextField(labelWithString: "OpenGlance")
        nameLabel.font = .systemFont(ofSize: 16, weight: .bold)
        nameLabel.textColor = .labelColor
        infoStack.addArrangedSubview(nameLabel)
        
        let verTitleStack = NSStackView()
        verTitleStack.orientation = .horizontal
        verTitleStack.alignment = .centerY
        verTitleStack.spacing = 4
        verTitleStack.translatesAutoresizingMaskIntoConstraints = false
        
        let versionTitle = NSTextField(labelWithString: "Version:")
        versionTitle.font = .systemFont(ofSize: 12, weight: .regular)
        versionTitle.textColor = .secondaryLabelColor
        verTitleStack.addArrangedSubview(versionTitle)
        
        verTitleStack.addArrangedSubview(versionLabel)
        versionLabel.font = .systemFont(ofSize: 12, weight: .medium)
        versionLabel.textColor = .secondaryLabelColor
        
        infoStack.addArrangedSubview(verTitleStack)
        logoStack.addArrangedSubview(infoStack)
        
        updateIntervalSelector.translatesAutoresizingMaskIntoConstraints = false
        updateIntervalSelector.widthAnchor.constraint(equalToConstant: 140).isActive = true
        
        tempUnitSelector.translatesAutoresizingMaskIntoConstraints = false
        tempUnitSelector.widthAnchor.constraint(equalToConstant: 140).isActive = true
        
        let sections = [
            SettingsFormSection(title: "App Information", rows: [
                SettingsFormRow(left: logoStack, right: nil)
            ]),
            SettingsFormSection(title: "System Settings", rows: [
                SettingsFormRow(left: autostartOnBootCheckbox, right: nil),
                SettingsFormRow(label: "Update Interval", subtitle: "Frequency of updating CPU, memory, and network metrics", control: updateIntervalSelector),
                SettingsFormRow(label: "Temperature Unit", subtitle: "Select temperature scale (Celsius, Fahrenheit, Kelvin)", control: tempUnitSelector),
                SettingsFormRow(left: advancedLoggingCheckbox, right: nil)
            ])
        ]
    ...
    */
    private func changeLogo() {
        if ThemeManager.isDarkTheme() {
            logoImage.image = NSImage(named: "OpenGlance_logo_white")
        } else {
            logoImage.image = NSImage(named: "OpenGlance_logo_black")
        }
    }
