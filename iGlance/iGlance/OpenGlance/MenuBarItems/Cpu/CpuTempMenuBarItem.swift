...
class CpuTempMenuBarItem: MenuBarItem {
    ...
    private func updateMenuBarIcon() {
        let tempDouble = AppDelegate.systemInfo.cpu.getCpuTemp()

        // If the temperature could not be read (-1.0) the SMC keys for this
        // machine are unavailable. Hide the menu bar item instead of rendering
        // a meaningless "-1°".
        if tempDouble < 0 {
            DDLogError("Could not read the cpu temperature. Hiding the CpuTempMenuBarItem.")
            self.statusItem.isVisible = false
            return
        }

        let temp = Int(tempDouble)
